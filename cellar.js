...
var Cellar = (function (_super) {
    ...
    Cellar.prototype.grow = function () {
        ...
        this.listen(lawn, 'http.start', function (app) { return _this.initialize_services(app); });
    };
    Cellar.prototype.initialize_services = function (app) {
        ...
        var lawn = this.vineyard.bulbs.lawn;
        lawn.listen_user_http('/vineyard/upload', function (req, res, user) { return _this.upload(req, res, user); });
        if (this.config.paths.cache && this.config.templates) {
            lawn.listen_user_http('/vineyard/cellar/:template/:guid.:ext', function (req, res, user) { return _this.file_download(req, res, user); }, 'get');
        }
    };
    Cellar.prototype.load_schema = function (name) {
        var path = require('path');
    ...
    Cellar.prototype.file_download = function (req, res, user) {
        ...
        return Cellar.file_exists(filepath).then(function (exists) {
            ...
            return _this.assure_folder(template_folder).then(function () { return Cellar.file_exists(source_file); }).then(function (exists) {
                if (!exists)
